...
**SUPERSEDED 2026-05-28 — automation now live.** LA was originally "manual phone-per-parish" because no clean automated source existed. Then Willie provided the `Sales_Tax_Compliance_TX_OK_LA_KS_AR.xlsx` workbook (LATA-sourced, 444 jurisdiction rows). That unlocked full automation.

**Current LA pipeline:**

```
Address → Mapbox geocode → lat/lng
  ↓
PostGIS spatial join (la_parishes, la_cities polygons from Census TIGER)
  ↓
LATA jurisdiction match (5-strategy: exact / prefix / parens / balance / parishwide)
  ↓
Sum: LA state 5% + parish_combined rate (typically 4-6%) + special districts
  ↓
Return combined rate + provenance
```

**Coverage:**
- 62 of 64 parishes automated via LATA data
- **Cameron + Jefferson** are LATA gaps (Cameron's page wasn't fetched; Jefferson has multiple sub-category rates LATA doesn't decompose). These return `low_confidence` and fall back to manual venue-pin.
- 6 LA address spot-checks pass end-to-end via `scripts/import_la_lata_rates.ts`:
  - New Orleans 10.000%, Baton Rouge 10.500%, Shreveport 9.600%, Lafayette 9.000%, Lake Charles 10.750%, Crowley 10.500%

**Tables:**
- `la_parishes` — 64 Census TIGER parish polygons + parish rate
- `la_cities` — 305 Census TIGER place polygons + city rate (FIPS-keyed)
- `la_lata_jurisdictions` — 444 LATA rows (parish + jurisdiction_name + combined_local_rate + components_breakdown + source_url)
- `la_special_districts` — hand-traced polygons (Phase LA.5; not blocking)
- `la_geocode_cache` — Mapbox response cache (365-day TTL)

**Lookup function:**
- `la_lookup_by_latlng(lat, lng)` — returns rate stack as (jurisdiction_name, jurisdiction_type, jurisdiction_rate) rows

**Client:**
- `src/lib/tax/laParishClient.ts` — wraps geocoder + RPC; mirrors the other 4 state-client signatures

**Maintenance — quarterly refresh:**

LATA rates change quarterly. To refresh:

1. Get an updated copy of the compliance workbook (typically from accounting firm or by re-running the workbook builder against current LATA pages)
2. Re-extract the JSON:
   ```bash
   python3 -c "
   import json
   from openpyxl import load_workbook
   wb = load_workbook('/path/to/Sales_Tax_Compliance_TX_OK_LA_KS_AR.xlsx', data_only=True, read_only=True)
   ws = wb['LA Parish Rates (Detailed)']
   rows = list(ws.iter_rows(values_only=True))
   header = [str(h) if h is not None else '' for h in rows[0]]
   data = []
   for r in rows[1:]:
       rec = {h: (r[i].isoformat()[:10] if hasattr(r[i], 'isoformat') else r[i]) for i, h in enumerate(header)}
       if rec.get('Parish'): data.append(rec)
   with open('scripts/la_lata_data.json', 'w') as f:
...
Idempotent — upserts replace stale rates. Migrations stay as-is.

**Reference sources (still useful for spot-checking individual addresses):**
- https://parishe-file.revenue.louisiana.gov/lookup/ (ASP.NET WebForms; manual UI only)
- https://lulstb.com/jurisdiction-list/ (parish names + contact info)
- https://rates.salestaxportal.com/public (LULSTB's free per-address lookup; works in a browser, API gated)

**Critical caveat — Cameron + Jefferson:**

These two parishes return `low_confidence` from the LA client. Two paths to fix:

**Path A — Pin a specific venue (faster):**
Use `/admin/tax-venues` to add a single Cameron or Jefferson venue with the verified combined rate from the parish tax collector. Only that venue gets covered.

**Path B — Add parish-level row to LATA table (covers entire parish):**
Once you have an authoritative rate from your accountant or the parish's published source, paste this in Supabase SQL Editor. The lookup function will then resolve any Cameron / Jefferson address using these rates.

```sql
-- Cameron Parish — replace COMBINED_LOCAL with the authoritative parish rate
-- (e.g., 0.0500 for 5.0%). State rate stays 0.05; total = local + state.
INSERT INTO public.la_lata_jurisdictions (
  parish_name, jurisdiction_name, jurisdiction_kind,
  components_breakdown, combined_local_rate, state_rate, total_rate,
  effective_date, source_url, source_verified_at, notes
) VALUES (
  'Cameron',
  'Balance of Parish',
  'balance',
  'Manual entry — fill in components from authoritative source',
  0.0500,   -- ← REPLACE with verified combined local rate
  0.05,
  0.1000,   -- ← combined + state
  '2026-04-01',
  'manual_entry_from_<source_name>',
  CURRENT_DATE,
  'Manually entered <YOUR NAME> 2026-XX-XX from <source>. Replace when LATA publishes Cameron.'
...
-- Jefferson Parish — Jefferson has multiple sub-category rates in LATA's data.
-- For Atlas's TPP sales, the "General Sales" rate applies. Get the current
-- value from your accountant or LULSTB; typical combined ~9.75%.
INSERT INTO public.la_lata_jurisdictions (
  parish_name, jurisdiction_name, jurisdiction_kind,
  components_breakdown, combined_local_rate, state_rate, total_rate,
  effective_date, source_url, source_verified_at, notes
  ...
  'Jefferson',
  'Parishwide - General Sales',
  'balance',
  'Manual entry — fill in components from authoritative source',
  0.0475,   -- ← REPLACE with verified combined local rate
  0.05,
  0.0975,
  '2026-04-01',
  'manual_entry_from_<source_name>',
  CURRENT_DATE,
  'Manually entered <YOUR NAME> 2026-XX-XX. Jefferson has multiple sub-category rates in LATA; this is the general TPP rate.'
) ON CONFLICT (parish_name, jurisdiction_name) DO UPDATE SET
  combined_local_rate = EXCLUDED.combined_local_rate,
  total_rate = EXCLUDED.total_rate,
  effective_date = EXCLUDED.effective_date,
...
After running, re-run a spot-check via `bun scripts/import_la_lata_rates.ts` — the script's spot-check section will hit Metairie / Lake Charles area test addresses and confirm the new rates apply.
