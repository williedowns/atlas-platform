...
ALTER TABLE public.contracts
  ADD COLUMN IF NOT EXISTS total_adjustment_amount numeric(10, 2) NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS total_adjustment_reason text;

ALTER TABLE public.contracts
  DROP CONSTRAINT IF EXISTS contracts_total_adjustment_cap;
ALTER TABLE public.contracts
  ADD CONSTRAINT contracts_total_adjustment_cap
  CHECK (abs(total_adjustment_amount) <= 5);

COMMENT ON COLUMN public.contracts.total_adjustment_amount IS
  'Admin-only post-tax adjustment to the contract total. Used to reconcile penny-precision mismatches (e.g. GreenSky funded $0.01 under the calculated total). Positive = customer owes more; negative = customer owes less. Tax and discount math are unaffected. Capped at abs(value) <= 5 by both API and DB CHECK.';
