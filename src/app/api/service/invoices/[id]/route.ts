...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.line_items !== undefined) {
    ...
    const subtotal = (body.line_items as any[]).reduce((s: number, i: any) => s + (Number(i.qty) * Number(i.unit_price)), 0);
    updates.subtotal = subtotal;
    const tax = body.tax_amount !== undefined ? Number(body.tax_amount) : 0;
    updates.total = subtotal + tax;
    if (body.tax_amount !== undefined) updates.tax_amount = tax;
  } else if (body.tax_amount !== undefined) {
    updates.tax_amount = Number(body.tax_amount);
    const { data: existing } = await supabase.from("service_invoices").select("subtotal").eq("id", id).single();
    updates.total = Number(existing?.subtotal ?? 0) + Number(body.tax_amount);
  }
  ...
  const { data, error } = await supabase
    ...
    .update(updates)
    .eq("id", id)
    .select("*, customer:customers(first_name,last_name,email), job:service_jobs(title,scheduled_date,job_type)")
    .single();

  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  // Send email when status → sent
  if (body.status === "sent") {
    try {
          ...
          html: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
            ...
            <div style="padding:24px;">
              ...
              <table style="width:100%;border-collapse:collapse;font-size:14px;margin:16px 0;">
                ...
                <tbody>${rows}</tbody>
                <tfoot>
                  <tr><td colspan="3" style="padding:8px;text-align:right;color:#64748b;">Subtotal</td><td style="padding:8px;text-align:right;">$${Number(data.subtotal).toFixed(2)}</td></tr>
                  ${Number(data.tax_amount) > 0 ? `<tr><td colspan="3" style="padding:8px;text-align:right;color:#64748b;">Tax</td><td style="padding:8px;text-align:right;">$${Number(data.tax_amount).toFixed(2)}</td></tr>` : ""}
                  <tr style="font-weight:bold;background:#f8fafc;"><td colspan="3" style="padding:8px;text-align:right;">Total Due</td><td style="padding:8px;text-align:right;color:#00929C;font-size:16px;">$${Number(data.total).toFixed(2)}</td></tr>
                </tfoot>
              </table>
