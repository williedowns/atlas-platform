...
import type { Dealer, Showroom } from "@/lib/manufacturer/mock-data";
...
export interface ShowroomNetworkData {
  totalShowrooms: number;
  totalDealers: number;
  avgShowroomsPerDealer: number;
  dealersWithMultipleShowrooms: number;
  showroomRevenueYtd: number;
  showRevenueYtd: number;
  distribution: { count: string; dealers: number }[];
  topByCount: Dealer[];
  expansionCandidates: Dealer[];
  regionData: { region: string; showrooms: number; dealers: number; avgPerDealer: number }[];
  topRooms: Showroom[];
  dealers: Dealer[];
}

export default function ShowroomsClient({
  data,
  atlasIsLive,
}: {
  data: ShowroomNetworkData;
  atlasIsLive: boolean;
}) {
  const {
    totalShowrooms,
