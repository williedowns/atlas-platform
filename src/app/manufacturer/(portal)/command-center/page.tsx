...
export default function CommandCenterPage() {
  ...
  const [livePulse, setLivePulse] = useState(NETWORK_STATS.revenueToday);

  useEffect(() => {
    const interval = setInterval(() => {
      setTickerIdx((i) => (i + 1) % Math.max(1, LAST_HOUR_CONTRACTS.length));
  ...
  const trend = revenueTrendLast30Days().slice(-14);
  const liveShows = SHOWS.filter((s) => s.status === "live");
  const topShows = [...liveShows].sort((a, b) => b.revenue - a.revenue).slice(0, 6);

  const currentTicker = LAST_HOUR_CONTRACTS.slice(tickerIdx, tickerIdx + 6);
    ...
    >
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-right">
          ...
          <p className="text-white text-5xl font-black tabular-nums">{NETWORK_STATS.totalDealers}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <LiveStat
          ...
          sub={`${NETWORK_STATS.unitsToday} units · updating live`}
          accent={MS_BRAND.colors.primary}
        />
        <LiveStat
          label="Last 60 Minutes"
          value={`${NETWORK_STATS.unitsLastHour} units`}
          sub={fmtCurrency(NETWORK_STATS.revenueLastHour)}
          accent={MS_BRAND.colors.accent}
        />
        <LiveStat
          label="Active Shows"
          value={`${liveShows.length}`}
          sub={`${liveShows.reduce((s, x) => s + x.contractsSigned, 0)} contracts at shows`}
          accent={MS_BRAND.colors.success}
        />
        <LiveStat
          label="Network Inventory"
          value={NETWORK_STATS.totalInventoryUnits.toLocaleString()}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white/5 backdrop-blur border border-white/10 rounded-xl p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <p className="text-white text-xl font-black">{liveShows.length}</p>
          </div>
          <div className="space-y-2">
            {topShows.map((s) => (
