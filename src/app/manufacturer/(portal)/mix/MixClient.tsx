...
export interface MixData {
  ...
  colorMix: Record<string, number>;
  regional: Record<string, { units: number; revenue: number; dealers: number }>;
}

export default function MixClient({ data }: { data: MixData }) {
  const modelData = MODEL_LINES.map((m) => ({
    name: m,
    shortName: m.replace("Michael Phelps", "MP").replace(" Swim Spa", ""),
    units: data.mix[m].units,
    revenue: data.mix[m].revenue,
    color: MS_BRAND.modelLineColors[m],
  }));
  const totalUnits = modelData.reduce((s, m) => s + m.units, 0);

  const colorData = Object.entries(data.colorMix).map(([name, v]) => ({ name, units: v }));

  const regionalModelMix = Object.entries(data.regional).map(([region, stats]) => ({
    region,
    avgUnitsPerDealer: Math.round(stats.units / Math.max(1, stats.dealers)),
    avgRevenuePerDealer: Math.round(stats.revenue / Math.max(1, stats.dealers)),
