...
export default function OverviewPage() {
  ...
  const top = topDealers(5);
  const bottom = bottomDealers(5);
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <KpiCard
          label="Revenue Today"
          value={fmtCurrency(NETWORK_STATS.revenueToday)}
          sublabel={`${NETWORK_STATS.unitsToday} units sold`}
          trend="up"
          trendValue="12%"
        ...
        <KpiCard
          ...
          sublabel={`${NETWORK_STATS.dealersWithMultipleShowrooms} multi-location dealers`}
          trend="up"
          trendValue="+14 YTD"
        ...
        <KpiCard
          label="YTD Units"
          value={fmtCompact(NETWORK_STATS.ytdUnits)}
          sublabel={fmtCurrency(NETWORK_STATS.ytdRevenue)}
          trend="up"
          trendValue="8%"
        ...
        <KpiCard
          ...
          value={NETWORK_STATS.atRiskDealers}
          sublabel={`Avg health: ${NETWORK_STATS.avgHealthScore}/100`}
          trend="down"
          trendValue="3"
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-slate-500">
              YTD network revenue · showrooms are {(NETWORK_STATS.showroomRevenueYtd / NETWORK_STATS.showRevenueYtd).toFixed(2)}x shows
            </p>
          </div>
        ...
        <div className="flex h-10 rounded-lg overflow-hidden">
          <div
            ...
            style={{
              width: `${(NETWORK_STATS.showroomRevenueYtd / (NETWORK_STATS.showroomRevenueYtd + NETWORK_STATS.showRevenueYtd)) * 100}%`,
              backgroundColor: MS_BRAND.colors.primary,
            }}
          >
            Showrooms · {fmtCurrency(NETWORK_STATS.showroomRevenueYtd)}
          </div>
          <div
            className="flex items-center justify-end pr-4 text-white text-sm font-bold"
            style={{
              width: `${(NETWORK_STATS.showRevenueYtd / (NETWORK_STATS.showroomRevenueYtd + NETWORK_STATS.showRevenueYtd)) * 100}%`,
              backgroundColor: MS_BRAND.colors.accent,
            }}
          >
            Shows · {fmtCurrency(NETWORK_STATS.showRevenueYtd)}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <SectionCard title="Live Shows Right Now" subtitle={`${liveShows.length} in progress`} viewAllHref="/manufacturer/shows">
          <div className="space-y-3">
            {liveShows.map((s) => (
              <div key={s.id} className="flex items-center justify-between pb-3 border-b border-slate-100 last:border-0 last:pb-0">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                    <p className="text-sm font-semibold text-slate-900 truncate">{s.dealerName}</p>
                  </div>
                  <p className="text-xs text-slate-500 truncate">{s.city}, {s.state}</p>
                </div>
            ...
            {top.map((d, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500">{d.city}, {d.state} · {d.tier}</p>
                </div>
            ...
            {bottom.map((d) => (
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: d.healthScore < 40 ? "#DC2626" : d.healthScore < 55 ? "#D97706" : "#F59E0B" }}
                >
                  {d.healthScore}
                </div>
                <div className="flex-1 min-w-0">
