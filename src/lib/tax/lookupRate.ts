...
export type LookupOutcome =
  ...
  | "requires_verification" // LA — caller must verify with parish
  | "no_data";        // miss
...
export async function lookupRate(args: LookupArgs): Promise<TaxRateLookupResult> {
  ...
  if (args.state === "LA") {
    return {
      ...
      effective_date: null,
      warning:
        "Louisiana home-rule parish — no automated rate lookup is reliable. " +
        "1) Identify the destination parish for this address. " +
        "2) Call the parish Sales Tax Department (state rate is 5%; combined rates typically 8.45-11.45%). " +
        "3) Verify state + parish + city + any tourism/convention/economic-development district rates. " +
        "4) Pin the venue via /admin/tax-venues so future sales at this address cache the rate. " +
        "Reference lookups (sanity check only — do not rely on these as the sole verification): " +
        "https://parishe-file.revenue.louisiana.gov/lookup/ , https://lulstb.com/ .",
      verified_by: null,
    };
  }
